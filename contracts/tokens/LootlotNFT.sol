...
contract LootlotNFT is ERC721Psi, ILootlotNFT, Ownable {
    
    error LootlotNFT_CurrentLocked();
    ...
    constructor(string memory name_, string memory symbol_) 
        ...
        _locked = true;
        baseURI = "https://playestates.mypinata.cloud/ipfs/QmWa24Eojed2MVbeNuRpTqQNuzhSa3rzvNjVkZnkpTubAh/";
    }
        ...
        string memory s2 = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        s1,
                        ', "description": "PlayEstates Lootlot NFT"}'                        
                    )
                )
            )
        );
