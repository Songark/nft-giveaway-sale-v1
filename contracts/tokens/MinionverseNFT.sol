...
contract MinionverseNFT is ERC721Psi, IMinionverseNFT, Ownable {
    
    error MinionverseNFT_CurrentLocked();
    ...
    string[] public typeNames = ["Flower", "Gargul", "Shaman", "Spider"];
    ...
    modifier validTokenType(TokenType tokenType) {
        require(tokenType >= TokenType.Flower && tokenType <= TokenType.Spider,
            "Invalid token type");
        _;
    }
        ...
        string memory s2 = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        s1,
                        ', "description": "PlayEstates Minionverse NFT"',
                        ', "attributes": [',
                        '{ "trait_type": "Price", "value": "5 USDC" }, ',
                        '{ "trait_type": "Minionverse Type", "value": "',
    ...
    override view {        
        if (from != address(0) && from != _treasury) {
            require(!_locked, "Minionverse is locked");
        }
    }
